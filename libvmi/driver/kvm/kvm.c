...
kvm_get_memory(
    ...
    uint32_t length)
{
    uint8_t *page_buffer = g_try_malloc0(length);
    if (!page_buffer)
    {
        return NULL;
    }
    ...
    MicrovmiStatus status = microvmi_read_physical(kvm_get_instance(vmi)->microvmi_context, paddr, page_buffer, length);
    ...
    vmi_init_data_t* UNUSED(init_data))
{
    ...
    // Since there is no general initialization for libmicrovmi we cannot probe anything here.
    ...
    switch (reg)
    {
        ...
        case RFLAGS:
            *value = microvmi_regs.x86._0.rflags;
            break;
        case CR0:
            *value = microvmi_regs.x86._0.cr0;
            break;
        case CR3:
            *value = microvmi_regs.x86._0.cr3;
            break;
        case CR4:
            *value = microvmi_regs.x86._0.cr4;
            break;
        case FS_BASE:
            *value = microvmi_regs.x86._0.fs_base;
            break;
        default:
            ret = VMI_FAILURE;
            break;
